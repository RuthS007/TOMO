...
export const BuddyTab: React.FC<BuddyTabProps> = ({
  ...
  const categories = [
    {
      ...
      label: "All Plans",
      icon: "✨",
      bg: "bg-white text-slate-700 border border-slate-200/80",
      activeBg: "bg-teal-700 text-white shadow-xs border border-teal-700",
    },
    {
      id: "study" as const,
      label: "Study Jam",
      icon: "📚",
      bg: "bg-violet-50 text-violet-800 border border-violet-200/60",
      activeBg: "bg-violet-600 text-white shadow-xs border border-violet-600",
    },
    {
      id: "lunch" as const,
      label: "Food & Boba",
      icon: "🥪",
      bg: "bg-teal-50 text-teal-800 border border-teal-200/60",
      activeBg: "bg-teal-700 text-white shadow-xs border border-teal-700",
    },
    {
      ...
      label: "Sports & Fun",
      icon: "🏀",
      bg: "bg-purple-50 text-purple-800 border border-purple-200/60",
      activeBg: "bg-purple-700 text-white shadow-xs border border-purple-700",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 pb-28 pt-3 px-4 max-w-md mx-auto relative select-none">
      ...
      <div className="flex items-center justify-between pt-1 pb-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-medium">
            Spontaneous meetups, study groups & campus plans
          </p>
        </div>
        ...
        <button
          ...
          className="px-3.5 py-1.5 rounded-full bg-teal-700 hover:bg-teal-800 text-white text-xs font-bold shadow-xs flex items-center gap-1.5 cursor-pointer transition active:scale-95"
        >
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full font-bold text-xs flex items-center gap-1.5 transition cursor-pointer shadow-xs shrink-0 active:scale-95 ${
      ...
      {/* Announcements List in Clean Realistic Campus Cards */}
      <div className="space-y-3.5 pb-8">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-3xl p-8 text-center shadow-xs border border-slate-200/80">
            <span className="text-3xl">🎈</span>
            <h3 className="font-bold text-sm text-slate-900 mt-2">
              No plans posted in this category yet
            </h3>
            <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto">
              Start something fun! Propose a study jam, lunch run, or gym session.
            </p>
            <button
              onClick={() => setShowCreateModal(true)}
              className="mt-4 px-4 py-2 rounded-full bg-teal-700 hover:bg-teal-800 text-white text-xs font-bold transition shadow-xs cursor-pointer"
            >
              Create Invitation
            </button>
          </div>
          ...
          filtered.map((ann) => {
            ...
            return (
              <div
                key={ann.id}
                className="bg-white rounded-3xl p-4.5 shadow-[0_4px_20px_-4px_rgba(15,23,42,0.06)] border border-slate-200/80 hover:border-slate-300 hover:shadow-md transition group"
              >
                {/* Host and Category */}
                <div className="flex items-center justify-between gap-2 mb-2">
                  ...
                  <span className="w-8 h-8 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-base">
                    {icon}
                  </span>
                </div>

                {/* Announcement Headline */}
                <h3 className="font-display font-extrabold text-sm text-slate-900 mb-2 leading-snug">
                  {ann.title}
                </h3>

                {/* Pills for Location & Time */}
                <div className="flex flex-wrap items-center gap-1.5 mb-3 text-[11px]">
                  <span className="px-2.5 py-1 rounded-full bg-teal-50 text-teal-800 border border-teal-200/60 font-semibold flex items-center gap-1">
                    <MapPin className="w-3 h-3 text-teal-600" />
                    <span>{ann.location}</span>
                  </span>
                  <span className="px-2.5 py-1 rounded-full bg-violet-50 text-violet-800 border border-violet-200/60 font-semibold flex items-center gap-1">
                ...
                <div className="flex items-center justify-between pt-2.5 border-t border-slate-100">
                  <div className="flex items-center -space-x-2">
                    ...
                    <span className="text-[11px] font-bold text-slate-500 pl-3">
                      {ann.participants.length}/{ann.targetCount} spots
                    </span>
                  </div>
                  ...
                  {hasJoined ? (
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/80 font-bold text-xs flex items-center gap-1 cursor-pointer hover:bg-rose-50 hover:text-rose-700 hover:border-rose-200 transition active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full font-bold text-xs transition cursor-pointer active:scale-95 ${
                        isFull
            ...
            <form onSubmit={handleCreate} className="space-y-3.5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 font-medium"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500"
                    required
                  />
                </div>
                <div>
                ...
                <div className="flex gap-2">
                  {[2, 3, 4, 6].map((num) => (
                    <button
                      ...
                      className={`flex-1 py-1.5 rounded-xl text-xs font-bold border cursor-pointer transition ${
                        newTargetCount === num
